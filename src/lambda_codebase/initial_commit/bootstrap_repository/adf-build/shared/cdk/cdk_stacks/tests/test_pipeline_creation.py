...
@patch("cdk_stacks.main.generate_default_pipeline")
def test_pipeline_generation_works_when_default_type_specified(mock):
    stack_input = {"input": {"params": {}}}
    stack_input["input"]["name"] = "test-stack"
...
@patch("cdk_stacks.main.generate_cdk_pipeline")
def test_pipeline_generation_works_when_cdk_type_specified(mock):
    stack_input = {"input": {"params": {}}}
    stack_input["input"]["name"] = "test-stack"
    stack_input["input"]["params"]["type"] = "cdk"
...
def test_pipeline_creation_outputs_as_expected_when_cdk_pipeline_using_codecommit_source():
    region_name = "eu-central-1"
    acount_id = "123456789012"
    ...
    stack_input["input"]["default_providers"]["source"] = {
        "provider": "codecommit",
        "properties": {"account_id": "123456789012", "repository": "spinnaker-access-strata"},
    }
    ...
    cloud_assembly = app.synth()
    print(cloud_assembly.stacks[0].template)
    resources = {k[0:-8]: v for k, v in cloud_assembly.stacks[0].template['Resources'].items()}
    code_pipeline = resources['codepipeline']
    assert code_pipeline['Type'] == "AWS::CodePipeline::Pipeline"
    assert len(code_pipeline["Properties"]["Stages"]) == 2
